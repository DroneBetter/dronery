...
smp=lambda f,*i: sum(map(f,*i));srmp=lambda f,*i: sum(starmap(f,*i))
...
redigits=lambda l,b: srmp(lambda i,d: d*b**i,enumerate(l))
...
subfact=lambda n: smp(lambda k: comb(~k,~n)*fact(k),range(n+1))
...
A132812=lambda n,k: comb(n,k)*comb(n,k-1)
catalan=A000108=lambda n: comb(2*n,n)//(n+1) #equivalently, catalanRecur=lru_cache(lambda n: smp(lambda k: catalanRecur(k)*catalanRecur(n+~k),range(n)) if n else 1)
...
Delta=forwardDifferences=lambda f,n=1: lambda k: smp(lambda i: comb(~i,~n)*f(k+i),range(n+1)) #nth iterate
...
rcycle_int=lambda r,n,k: smp(lambda i: comb(int(n-r),int(k-r)+i)*cycle(int(k-r)+i,int(k-r))*rising(r,int(n-k)-i),range(int(n-k)+1)) #(eq. 27 and thus 56)
...
cycpart=lambda n,k: smp(lambda c: (n-1)**c         *comb(k+c>>1,c)           *smp(lambda t: t.bit_count()==k+c>>1     and prod(map(lambda i: (i*(n+~i),1)[t>>i&1],range(n>>1))),range(1<<(n>>1))),range(k&1,k+2,2)) #O(n*sqrt(2)**n) time
cycpart=lambda n,k: smp(lambda c: (n-1)**(c<<1|k&1)*comb((k+1>>1)+c,c<<1|k&1)*smp(lambda t: t.bit_count()==(k+1>>1)+c and prod(map(lambda i: (i*(n+~i),1)[t>>i&1],range(n>>1))),range(1<<(n>>1))),range((k>>1)+1)) #O(n*sqrt(2)**n) time
...
evensubset=lambda n,k: not(n or k) or ~n&1 and k<=n>>1 and 2*smp(lambda j: (-1)**(k-j)*comb(2*k,k-j)*j**n,range(k+1))//(fact(k)<<k) #subset triangle but the subsets are of even cardinality
evensubset=lambda n,k: ~n&1 and k<=n>>1 and int(comb(2*k,k)*forwardDifferences(lambda i: frac(i**n,fact(k+i)//fact(i)),k)(0))>>k-1 if k else not n #in terms of forward differences
...
bij=lambda n,k: smp(lambda j: (-1)**(n+k-j)*comb(n,j)*rcycle(j+1,n+1,n+1-k),range(n-k+1))
bij=lambda n,k: smp(lambda j: (-1)**(j-k)*falling(n,j)*smp(lambda l: frac(comb(n-l,j-l),fact(l))*cycle(l,j-k),range(j-k,j+1)),range(k,n+1))
A058349=lambda k: smp(lambda n: bij(n,k),range(k:=k-1,2*k+1)) #conjectural! (for now) #for miscellaneouscuriosityheads
A058349=lambda k: smp(lambda i: smp(lambda d: comb(2*i,i+d)*d**(i+k-1)*(-1)**(k+~d),range(-i,i+1))//fact(i),range(k)) #not conjectural :-)
A048174=lambda k: smp(lambda i: smp(lambda d: comb(2*i,i+d)*(d+1)**(i+k-1)*(-1)**(k+~d),range(-i,i+1))//fact(i),range(k)) #not conjectural :-)
A392857=multA058349=lambda n,k: smp(lambda j: fact(2*j)//fact(j)*comb(n+j-1,k-1)*rsubset_int(-j,n-k,j),range(n-k+1))
A058349=lambda n: multA058349(n,1)

egfexp=lambda f: lambda n: fact(n)*smp(lambda p: frac(comb(len(p),*map(rgetitem(1),rle(p)))*prod(map(lambda k: frac(f(k),fact(k)),p)),fact(len(p))),accel_asc(n))
faa=lambda f,g,egf=False: lambda n: smp(lambda p: frac(f(len(p)),fact(len(p)) if egf else 1)*prod(map(g,p))*(comb(n,*p) if egf else 1)*comb(len(p),*map(rgetitem(1),rle(p))),accel_asc(n)) #Faà di Bruno is so cool
...
partmute=lambda K,P,p: (comb(len(p),*map(rgetitem(1),rle(p))) if K==2 else len(sap(lambda p: tuple(min(map(lambda i: p[i:]+p[:i],range(len(p))))),multiset_permutations(p))) if K else 1)*(prod(map(fact,p)) if P==2 else prod(map(lambda i: fact(i-1),p)) if P else 1)
...
A285824help=lambda n,i,p: smp(lambda j: comb(n,*j*(i,),n-i*j)//fact(j)**2*A285824help(n-i*j,i-1,p+j)*x**j,range(n//i+1)) if n and i>1 else fact(n+p)//fact(n)*x**n
A285824internal=lambda n,k,i,p: smp(lambda j: comb(n,*j*(i,),n-i*j)*A285824internal(n-i*j,k-j,i-1,p+j)//fact(j)**2,range(min(n//i,k)+1)) if k and n>=k and i>1 else int(k==n) and fact(n+p)//fact(n)
...
A285824internalk2=lambda n,i,p: (n==2 or smp(lambda i:(i>n>>1)*2*comb(n,i)+(~n&1 and i==n>>1)*comb(2*i-1,i),range(i+1)))*fact(p+2)>>1
A285824internalk3=lambda n,i,p: (A285824internalk3(n,i-1,p)+comb(n,i)*(n==2 or smp(lambda j:(j>n-i>>1)*2*comb(n-i,j)+(~n-i&1 and j==n-i>>1)*comb(2*j-1,j),range(i)))+comb(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)//4+comb(n,i,i,i)*(n==3*i)//36 if n>=3 and i>1 else n==3)*fact(p+3)//6
A285824internalk3=lambda n,i,p: A285824internalk3(n,i-1,p)+comb(n,i)*((n-i==2 or smp(lambda j:(j>n-i>>1)*2*comb(n-i,j)+(~n-i&1 and j==n-i>>1)*comb(2*j-1,j),range(i)))*fact(p+3)>>1)+comb(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)*fact(p+3)//4+comb(n,i,i,i)*(n-3*i==0)*fact(p+3)//36 if n>=3 and i>1 else (3==n)*fact(n+p)//fact(n)
A285824row=lambda n: A285824help(n,n,0)
A285824=lambda n,k: A285824internal(n,k,n-k+1,0)
#lambda n: int(bool(n)) #col 1
A285917=lambda n: n and (1<<n)-(~n&1 and comb(n,n>>1)>>1)-2 #col 2 #lambda n: A285824internal(n,2,n-1,0)
A285918=lambda n: A285824internalk3(n,n-2,0)
A285918internal=lambda n,i,p=0: A285918internal(n,i-1)+comb(n,i)*A285824internal(n-i*j,k-j,i-1,p+j)//4+comb(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)*3//2+comb(n,i,i,i)*(n-3*i==0)//6 if n>=3 and i>1 else int(3==n)
A285918=lambda n: A285918internal(n,n-2)


A285918=lambda n: A285824internalk3(n,n-2,0)
A285824internalk3=lambda n,i,p: A285824internalk3(n,i-1,p)+comb(n,i)*A285824internal(n-i,2,i-1,p+1)+comb(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)*fact(p+3)//4+comb(n,i,i,i)*(n-3*i==0)*fact(p+3)//36 if n>=3 and i>1 else (3==n)*fact(n+p)//fact(n)


#these things from https://web.archive.org/web/20170202003812/http://www.dms.umontreal.ca/~andrew/PDF/BinCoeff.pdf
choosemodp=lambda n,k,p: modprod(p)(starmap(comb,zip_longest(digits(n,p),digits(k,p),fillvalue=0))) #Lucas's theorem
chooseval=lambda n,k,p: (digsum(k,p)+digsum(n-k,p)-digsum(n,p))//(p-1) #val(p)(comb(n,k))
chooseLastOnDig=lambda n,k,p: (-1)**chooseval(n,k,p)*modprod(p)(starmap(lambda n,k,r: fact(n)*pow(fact(k)*fact(r),-1,p),zip_longest(digits(n,p),digits(k,p),digits(n-k,p),fillvalue=0)))%p #comb(n,k)//p**chooseval(n,k,p)%p
chooseLastqOnDigs=lambda n,k,p,q: (p==2<q or (-1)**XORsum(map(lambda i: n//p**(q-1)%p**i<k//p**(q-1)%p**i,range(1,ilog(n,p)+2-q))))*modprod(p**q)(starmap(lambda n,k,r: factxcept(n,p,q)*pow(factxcept(k,p,q)*factxcept(r,p,q),-1,p**q),zip_longest(digruns(n,p,q),digruns(k,p,q),digruns(n-k,p,q),fillvalue=0)))%p**q #comb(n,k)//p**chooseval(n,k,p)%p**q #e_{q-1} in paper's formula is actually (-1)**∑_{i=1..⌊logₚ(n)⌋+1-q}(⌊n/p⌋𐞥⁻¹%pⁱ<⌊k/p⌋𐞥⁻¹%pⁱ)

subCarries=lambda x,y,c=0: sub(x^y,y:=(y&~x)<<1,c+y.bit_count()) if y else (x,c) #performs subtraction in binary, recording carries; subCarries(n,k)=val_2(comb(n,k))
...
#firstchoose,invchoose are in comb.py
