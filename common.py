...
def accel_asc(n): #(thank you https://jeromekelleher.net/generating-integer-partitions.html ;-)
    if n:
        ...
        while k:
            ...
            a[k]=y+1
            yield a[:k+1]
    else: yield [] #returning ([],) rather than ([0],) makes many identities nicer; with it, choose(n-1,k-1)=sum(map(lambda p: len(p)==k and choose(len(p),*map(rgetitem(1),rle(p))),accel_asc(n))) works for choose(-1,-1)=1
...
closure=close=lambda s,g: Y(lambda f: lambda s,g: s if (n:=set(starmap(g,product(s,repeat=2))))==s else f(n,g))(set(s),g)
...
subfact=lambda n: sum(map(lambda k: choose(~k,~n)*fact(k),range(n+1)))
...
falling=lambda n,k: (frac((-1)**-k*fact(k+~n),fact(~n)) if k<0 else n<0 and (-1)**k*fact(k+~n)//fact(~n)) if n<k else (n>=0 and frac(fact(n),fact(n-k)) if k<0 else fact(n)//fact(n-k))
...
catalan=A000108=lambda n: choose(2*n,n)//(n+1) #equivalently, catalanRecur=lru_cache(lambda n: sum(map(lambda k: catalanRecur(k)*catalanRecur(n+~k),range(n))) if n else 1)
catalanAccum=lambda n: tuple(accumulate(map(lambda k: catalan(k)*catalan(n+~k),range(n))))
from bisect import bisect
catalanSample=lambda n: Y(lambda f: lambda n: [f(b:=bisect(catalanAccum(n),randrange(catalan(n)))),f(n+~b)] if n else 0)(n) #n is the number of brackets

superfact=lambda n: prod(map(fact,range(n)))
superchoose=lambda n,k: superfact(n)//(superfact(k)*superfact(n-k))

forwardDifferences=lambda f,n=1: lambda k: sum(map(lambda i: choose(~i,~n)*f(k+i),range(n+1))) #nth iterate

#Stirling numbers of the second kind
subset_int=lambda n,k: forwardDifferences(lambda i: i**n,k)(0)//fact(k)
#subset_int=lambda n,k: sum(map(lambda i: choose(~i,~k)*i**n,range(k+1)))//fact(k)
#subset_int=lambda n,k: sum(map(lambda i: choose(-i,-k)*i**(n-1),range(1,k+1)))//fact(k-1) #decrement choose by removing mutual factors for extremely marginal speedup
...
#cycle_int=lambda n,k: sum(map(lambda i: choose(-k,-i)*choose(2*n-k,i)*subset(i-k,i-n),range(n,2*n+1-k)))
...
subset=lambda n,k: int(k<=n and (cycle(-k,-n) if n<0 else subset_int(n,k) if k>0 else n==0==k))
cycle=lambda n,k: int(k<=n and (subset(-k,-n) if n<0 else cycle_int(n,k) if k>0 else n==0==k))
#cycle=lambda n,k: sum(map(lambda t: t.bit_count()==n-k and prod(map(lambda i: (1,i+1)[t>>i&1],range(n-1))),range(2**(n-1)))) if n else not k #O(n*2**n) time
'''cycpart=lambda n,k: sum(map(lambda t: digsum(t,3)==k and prod(map(lambda i: (i*(n+~i),n-1,1)[t//3**i%3],range(n>>1))),range(3**(n>>1)))) #O(n*sqrt(3)**n) time
cycpart=lambda n,k: sum(map(lambda c: (n-1)**c         *choose(k+c>>1,c)           *sum(map(lambda t: t.bit_count()==k+c>>1     and prod(map(lambda i: (i*(n+~i),1)[t>>i&1],range(n>>1))),range(1<<(n>>1)))),range(k&1,k+2,2))) #O(n*sqrt(2)**n) time
cycpart=lambda n,k: sum(map(lambda c: (n-1)**(c<<1|k&1)*choose((k+1>>1)+c,c<<1|k&1)*sum(map(lambda t: t.bit_count()==(k+1>>1)+c and prod(map(lambda i: (i*(n+~i),1)[t>>i&1],range(n>>1))),range(1<<(n>>1)))),range((k>>1)+1))) #O(n*sqrt(2)**n) time
...
evensubset=lambda n,k: not(n or k) or ~n&1 and k<=n>>1 and 2*sum(map(lambda j: (-1)**(k-j)*choose(2*k,k-j)*j**n,range(k+1)))//(fact(k)<<k) #subset triangle but the subsets are of even cardinality
...
eulerian =lambda n,k,ind=1: sum(map(lambda i: (-1)**   i *choose(n+1,i)*(k+1-ind-i)**n,range(k+1-ind))) if k>=ind else not n #ind=0 for Euler/Knuth's convention, ind=1 for Comtet's
#eulerian2=lambda n,k: sum(map(lambda i: (-1)**   i *choose(2*n+1,i)*cycle(n+m-i,m-i),range(m:=n-k+1))) #do not use this one; current cycle implementation less efficient than its subset
eulerian2=lambda n,k: sum(map(lambda i: (-1)**(k-i)*choose(2*n+1,k-i)*subset(n+i,i),range(k+1))) #second-order Eulerians, as seen in the numerators of both Stirling triangles' diagonals' g.f.s' nums
#subset(d+k,k)=sum(map(lambda i: choose(2*d+k-i,2*d)*eulerian2(d,i),range(d+1)))
# cycle(d+k,k)=sum(map(lambda i: choose(d+k+i-1,2*d)*eulerian2(d,i),range(d+1)))
harmonic=harm=lambda n,o=1: frac(sum(map(lambda i: frac(1,i**o),range(1,n+1)))) #put here in particular because (for k>=2) cycle(n,k)=sum(map(lambda i: harm(n-1,i+1)*(-1)**i*cycle(n,k+~i),range(k)))//(k-1)
A058349=lambda k: sum(map(lambda n: sum(map(lambda i: (-1)**i*cycle(n+1,n+1-i)*sum(map(lambda j: (-1)**(n-j)*choose(n,j)*subset(k-i+j,j),range(n+1))),range(n+1))),range(k:=k-1,2*k+1))) #conjectural! (for now) #for miscellaneouscuriosityheads
egfexp=lambda f: lambda n: fact(n)*sum(map(lambda p: frac(choose(len(p),*map(rgetitem(1),rle(p)))*prod(map(lambda k: frac(f(k),fact(k)),p)),fact(len(p))),accel_asc(n)))
faa=lambda f,g,egf=False: lambda n: sum(map(lambda p: frac(f(len(p)),fact(len(p)) if egf else 1)*prod(map(g,p))*(choose(n,*p) if egf else 1)*choose(len(p),*map(rgetitem(1),rle(p))),accel_asc(n))) #Faà di Bruno is so cool
#A048172=lambda n: egfexp(A058349)(n)-(n==0)-A058349(n)
A000262=egfexp(lambda n: n and fact(n))
bell=lambda n: sum(map(lambda k: subset(n,k),range(n+1)))
subbell=lambda n: sum(map(lambda k: choose(~k,~n)*bell(k),range(n+1)))

from sympy.utilities.iterables import multiset_permutations,multiset_partitions #note latter is equivalent to accel_asc
#ordertitions=lambda C,K,P: lambda n,k: sum(map(lambda p: len(p)==k and (((choose(len(p),*map(rgetitem(1),rle(p))) if K==2 else len(sap(lambda p: tuple(min(map(lambda i: p[i:]+p[:i],range(k)))),multiset_permutations(p)))) if C else fact(len(p) if K==2 else len(p)-1)) if K else 1)*(prod(map(fact,p)) if P==2 else prod(map(lambda i: fact(i-1),p)) if P else 1),accel_asc(n))) if k else int(not n)
#ordertitions=lambda K,P: lambda n,k: sum(map(lambda p: len(p)==k and (1 if K==0 else choose(len(p),*map(rgetitem(1),rle(p))) if K==2 else len(sap(lambda p: tuple(min(map(lambda i: p[i:]+p[:i],range(k)))),multiset_permutations(p))))*(1 if P==0 else prod(map(fact,p)) if P==2 else prod(map(lambda i: fact(i-1),p))),accel_asc(n))) if k else int(not n)
...
ordertitions=lambda G,K,P: lambda n,k: (sum(map(lambda p: partmute(K,P,tap(len,p)),(id if G==2 else saph(lambda p: min(map(lambda j: tuple(sorted(tap(compose(taph(lambda i: (i+j)%n),sorted,tuple),p))),range(n)))))(multiset_partitions(range(n),k)))) if G else sum(map(lambda p: partmute(K,P,p),filter(lambda p: len(p)==k,accel_asc(n))))) if k else int(not n)

A285824help=lambda n,i,p: sum(map(lambda j: choose(n,*j*(i,),n-i*j)//fact(j)**2*A285824help(n-i*j,i-1,p+j)*x**j,range(n//i+1))) if n and i>1 else fact(n+p)//fact(n)*x**n
A285824internal=lambda n,k,i,p: sum(map(lambda j: choose(n,*j*(i,),n-i*j)*A285824internal(n-i*j,k-j,i-1,p+j)//fact(j)**2,range(min(n//i,k)+1))) if k and n>=k and i>1 else int(k==n) and fact(n+p)//fact(n)
...
A285824internalk2=lambda n,i,p: (n==2 or sum(map(lambda i:(i>n>>1)*2*choose(n,i)+(~n&1 and i==n>>1)*choose(2*i-1,i),range(i+1))))*fact(p+2)>>1
A285824internalk3=lambda n,i,p: (A285824internalk3(n,i-1,p)+choose(n,i)*(n==2 or sum(map(lambda j:(j>n-i>>1)*2*choose(n-i,j)+(~n-i&1 and j==n-i>>1)*choose(2*j-1,j),range(i))))+choose(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)//4+choose(n,i,i,i)*(n==3*i)//36 if n>=3 and i>1 else n==3)*fact(p+3)//6
A285824internalk3=lambda n,i,p: A285824internalk3(n,i-1,p)+choose(n,i)*((n-i==2 or sum(map(lambda j:(j>n-i>>1)*2*choose(n-i,j)+(~n-i&1 and j==n-i>>1)*choose(2*j-1,j),range(i))))*fact(p+3)>>1)+choose(n,i,i)*(n-2*i==1 or i-1>=n-2*i>0)*fact(p+3)//4+choose(n,i,i,i)*(n-3*i==0)*fact(p+3)//36 if n>=3 and i>1 else (3==n)*fact(n+p)//fact(n)
...
chinese=lambda m,c: int.__rmod__(pr:=prod(m:=tuple(m)),sum(map(lambda m,c: c*pow(p:=pr//m,-1,m)*p%pr,m,c))) #0≤x<prod(m) such that all(x%m[i]==c[i])
#2-argument version:
...
class fenwick:
  ...
  sum=lambda f,i=None: f.sum(len(f)) if i==None else (i>0 and sum(map(f.tree.__getitem__,redumulate(lambda i,_: (i&i+1)-1,range(i.bit_count()-1),i-1)))) #upper-exclusive
